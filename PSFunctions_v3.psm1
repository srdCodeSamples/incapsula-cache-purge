...
Function Write-CSTMLog
{
    ...
    foreach ($item in $Message)
        ...
        if($Append)
        {
            Out-File -FilePath $FilePath -InputObject "[$(Get-Date (Get-Date).ToUniversalTime() -Format "yyyy/MM/dd HH:mm:ss.fffZ")][$Type] $item" -Append
        }
        else
        {
            Out-File -FilePath $FilePath -InputObject "[$(Get-Date (Get-Date).ToUniversalTime() -Format "yyyy/MM/dd HH:mm:ss.fffZ")][$Type] $item"
        }
    }
}